...
def decode_token(token: str) -> dict:
    ...
    try:
        ...
        raise HTTPException(status_code=401, detail=f"Invalid authentication credentials: {str(e)}") from e
...
async def get_current_user_id(credentials: HTTPAuthorizationCredentials = Depends(security)) -> int:
    ...
    try:
        ...
        raise HTTPException(status_code=401, detail="Invalid token: user_id must be an integer") from e
